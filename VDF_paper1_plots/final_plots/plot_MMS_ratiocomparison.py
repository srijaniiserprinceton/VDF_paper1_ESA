...
for i, E_idx in enumerate(range(16,22)):
    ...
    ratio = (logvv - StepII_bundle['fine_from_fine'][E_idx].T) / StepII_bundle['fine_from_fine'][E_idx].T
...
ratio = np.abs(logvv[tstamp, :, :, theta_lr_slice] - StepII_bundle['fine_from_fine'][:,theta_lr_slice,:]) * 3.0
# ax[2].set_facecolor('black')
ax[2].pcolormesh(VX[:,:,theta_lr_slice], VY[:,:,theta_lr_slice], ratio,
               vmin=0, vmax=3, cmap='inferno', rasterized=True)
ax[2].set_xlim([-1500,1500])
ax[2].set_ylim([-1500,1500])
ax[2].set_aspect('equal')
ax[2].set_title(r'$3 \times |f_{\mathrm{MMS}} - f_{\mathrm{rec}}|$', fontsize=18, fontweight='bold')
...
plt.savefig('Cart_slice_ratiocompare.pdf')