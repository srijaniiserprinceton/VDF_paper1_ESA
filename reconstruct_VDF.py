...
def reconstruct_from_SolO_SphericalHarmonics(time_idx):
    #=============STEP I: Finding effective axis of gyrotropic across all relevant shells===========================#
    locate_axis.find_gyroaxis(rec_dict, time_idx, Nrows=4, Ncols=8)

    misc_funcs.gen_SH_scipy(rec_dict, time_idx)
...
def reconstruct_from_MMS_SphericalHarmonics(time_idx):
    ...
    locate_axis.find_gyroaxis(rec_dict, time_idx, Nrows=4, Ncols=8)     # This generates mu_phi, mu_theta, and phi_theta_cen  
                                                                        # and stores them into rec_dict.

    #--------------------------------------------saving the theta and phi grid -------------------------------------#
    # StepI_bundle = sph2slep.get_StepI_SHdict(DATA.PHI[0,:,0], DATA.THETA[0,0], instrument=instrument)
    misc_funcs.gen_SH_scipy(rec_dict, time_idx)
    # misc_funcs.gen_SH(rec_dict)

    #=============STEP II: Decomposing 3D measured VDF into Slepians on polar caps (gyrotropic)======================#
    StepII_bundle = VDF_rec_polarcaps.VDF_rec_polarcaps_SphericalHarmonics(rec_dict, time_idx, rcond=rcond_polcap)
    # return(StepII_bundle)
    # return(StepII_bundle)
    # sys.exit()
    #=============STEP III: Decomposing 2D gyrotropized VDF into Slepians in 2D (V{perp} vs V{||})===================#
...
def calc_moments_MMS_SH(time_idx, mask_noisy=False):
    ...
    REC_VDF = np.transpose(np.power(10, StepII_bundle.fine_from_fine) * 1e12 * rec_dict.VDF_minval_true[time_idx, None, None, None], [0, 2, 1])
    ...
    DATA_theta, DATA_phi = StepII_bundle.ESA_THETA[time_idx,0,0,:], StepII_bundle.ESA_PHI[time_idx,0,:,0]
    REC_theta, REC_phi   = StepII_bundle.SLEP_THETA, StepII_bundle.SLEP_PHI
...
if __name__=='__main__':
    ...
    sigma_val = 0.35                # Gaussian smoothing smoothness parameter.
    ...
    if(datascan_mode):
        ...
        for time_idx in tqdm(range(79, 80)):  # 481
            ...
            # tmp_vdf[tmp_vdf == 1.0] = np.nan
            # mask = (~np.isnan(np.transpose(tmp_vdf, [0,2,1]))).astype('float')
            # ss = 0.25
            # mask_sm1 = gaussian_filter(gaussian_filter(mask, sigma=ss, axes=(2), mode='wrap'),sigma=ss, axes=1)
            # mask_sm1[mask_sm1 > 0] = 1.0
            # mask_sm1[mask_sm1 == 0] = 16
            # mask_sm = gaussian_filter(mask_sm1, sigma=sigma_val)
            # mask_sm = mask * 1.0
            ...
            # superresolving the mask (for plotting purposes)
            # mask_sr = misc_funcs.interpolate_mask(mask_sm, NEmesh, NPmesh, NTmesh)
            # swapping theta and phi axes
            # mask_sr = np.moveaxis(mask_sr, 1, 2)
            ...
            VDF_3D_bundle = {}
            VDF_3D_rec = (8*np.tanh((VDF_3D_rec - np.log10(0.98)) / np.log10(1.1)) - 8) + VDF_3D_rec
            VDF_3D_bundle['VDF_3D_rec'] = VDF_3D_rec
            VDF_3D_bundle['VX'] = VX
            VDF_3D_bundle['VY'] = VY
            VDF_3D_bundle['VZ'] = VZ
            VDF_3D_bundle['E'] = np.power(10, lnE_mesh)
            ...
            StepII_bundle_plotdict['smooth_vdf'] = StepII_bundle.smooth_vdf
