...
if __name__=='__main__':
    ...
    ax[0].contourf(pp_orig, tt_orig, vv, cmap='rainbow', rasterized=True)#, levels=levels)
    ax[0].set_xlim([0,360])
    ax[0].set_ylim([0,180])
    ...
    ax[1].hist(phi_theta_cen[:,1], bins=5, range=(165, 205), facecolor='grey')
    ax[2].hist(phi_theta_cen[:,2], bins=5, range=(73, 95), orientation='horizontal', facecolor='grey')
    ...
    for ii, res_idx in enumerate(np.arange(8,33,6)):
        ax[3].contourf(pp_orig[:res_idx+1] + ii * 270, tt_orig[:res_idx+1], vv[:res_idx+1],
                       cmap='rainbow', rasterized=True)
        if(res_idx < 32):
            ax[3].contourf(pp_orig[res_idx:] + ii * 270, tt_orig[res_idx:], vv[res_idx:],
                        cmap='rainbow', rasterized=True, alpha=0.2)
        # ax[3].set_ylim([0,180])
    ...
    E_idx_min, E_idx_max = 8, 22
    phi_theta_mean = []
    for res_idx in range(8,32,3):
        phi_theta_cen = []
        for i, E_idx in enumerate(np.arange(E_idx_min, E_idx_max+1)):
            E = Energy[time_idx, E_idx] / 1.602e-19
            t, p, vv = Theta, Phi[time_idx, :res_idx], VDF[time_idx, :, :, E_idx]
            pp, tt = np.meshgrid(p, t, indexing='ij')
    ...
    for ii, res_idx in enumerate(np.arange(8,33,6)):
        ax[3].plot(phi_theta_mean[res_idx,1] + 270 * ii, phi_theta_mean[res_idx, 2], '.r')
        ax[3].plot(phi_theta_mean[5,1] + 270 * ii, phi_theta_mean[5, 2], 'xk')
    ...
    plt.savefig('plots/Fig1_MMS.pdf')
